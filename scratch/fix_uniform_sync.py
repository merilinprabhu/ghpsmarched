...
# Replace renderActiveUniformView, renderUniformCards, renderUniformTable, autoSaveRecord, toggleUniformSet, handleCardSizeChange
target_pattern = r'function renderActiveUniformView\(\)[\s\S]*?async function autoSaveRecord\(studentId\)'

replacement = """function renderActiveUniformView() {
      ...
      // Always render both so that elements exist for auto-saving and print drawers
        ...
        const cardHtml = `
          <div class="glass-card rounded-2xl p-4 border border-white/10 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-3.5" data-card-u-id="${s.id}">
            ...
            <div class="flex items-center justify-between pt-2 border-t border-white/10 text-xs">
              <span class="text-[10px] text-slate-400">ಸ್ಥಿತಿ: <strong id="card-status-text-${s.id}" class="${(s1 && s2) ? 'text-emerald-400' : 'text-amber-400'}">${(s1 && s2) ? 'ಎರಡೂ ನೀಡಲಾಗಿದೆ' : (s1 || s2 ? '1 Set ನೀಡಲಾಗಿದೆ' : 'ಬಾಕಿ')}</strong></span>
    ...
    function toggleUniformSet(studentId, setNum) {
      ...
      const s1 = document.getElementById(`set1-${studentId}`)?.checked;
      const s2 = document.getElementById(`set2-${studentId}`)?.checked;
      const statText = document.getElementById(`card-status-text-${studentId}`);
      if (statText) {
        statText.className = (s1 && s2) ? 'text-emerald-400' : 'text-amber-400';
        statText.innerText = (s1 && s2) ? 'ಎರಡೂ ನೀಡಲಾಗಿದೆ' : (s1 || s2 ? '1 Set ನೀಡಲಾಗಿದೆ' : 'ಬಾಕಿ');
      }
        ...
        html += `
          <tr class="hover:bg-slate-50/5 border-b border-white/5 text-slate-300 font-medium">
            ...
            <td class="p-2.5 bg-indigo-500/[0.01]">
              <select id="size-${s.id}" onchange="handleTableSizeChange('${s.id}', this.value)" class="bg-black/60 border border-white/10 rounded-xl px-2.5 py-1 text-xs font-bold text-indigo-300 focus:outline-none cursor-pointer">
                ${sizeOptions}
              </select>
            </td>

            <td class="p-2.5 bg-indigo-500/[0.01] border-x border-white/5 text-center">
              <input type="checkbox" id="set1-${s.id}" onchange="handleTableCheckboxChange('${s.id}', 1, this.checked)" ${s1 ? 'checked' : ''} class="w-5 h-5 rounded text-indigo-650 bg-black border-white/10 cursor-pointer transform scale-110">
            </td>

            <td class="p-2.5 bg-indigo-500/[0.01] border-x border-white/5 text-center">
              <input type="checkbox" id="set2-${s.id}" onchange="handleTableCheckboxChange('${s.id}', 2, this.checked)" ${s2 ? 'checked' : ''} class="w-5 h-5 rounded text-indigo-650 bg-black border-white/10 cursor-pointer transform scale-110">
            </td>
    ...
    function handleTableSizeChange(studentId, sizeVal) {
      const cardSelect = document.getElementById(`card-size-${studentId}`);
      if (cardSelect) cardSelect.value = sizeVal;
      autoSaveRecord(studentId);
    }

    function handleTableCheckboxChange(studentId, setNum, isChecked) {
      const btn = document.getElementById(`card-set${setNum}-btn-${studentId}`);
      if (btn) {
        if (isChecked) {
          btn.className = "py-2.5 px-3 rounded-xl font-black text-xs transition cursor-pointer flex items-center justify-center gap-1.5 bg-emerald-600 text-white shadow-md border-0 ring-2 ring-emerald-500/30";
...
u_code = re.sub(target_pattern, replacement, u_code)

with open('UniformDistribution.html', 'w', encoding='utf-8') as f:
    f.write(u_code)

print("Synchronized and fixed UniformDistribution.html card view")
