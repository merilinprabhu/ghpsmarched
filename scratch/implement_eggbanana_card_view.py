...
export_new = """        <!-- Export Buttons / Bulk Action -->
        <div class="flex gap-2 items-center flex-wrap">
          ...
          <div class="flex items-center gap-1 p-1 bg-black/40 border border-white/5 rounded-xl">
            <button type="button" id="btn-eb-cards" onclick="setEggBananaViewMode('cards')" class="px-3 py-1.5 rounded-lg text-xs font-black transition cursor-pointer flex items-center gap-1.5 bg-indigo-600 text-white shadow-xs border-0">
              <i class="fa-solid fa-grip-vertical"></i> <span>ಕಾರ್ಡ್ ವ್ಯೂ</span>
            </button>
            <button type="button" id="btn-eb-table" onclick="setEggBananaViewMode('table')" class="px-3 py-1.5 rounded-lg text-xs font-black transition cursor-pointer flex items-center gap-1.5 text-slate-400 hover:text-white border-0 bg-transparent">
              <i class="fa-solid fa-table-cells"></i> <span>ಕೋಷ್ಟಕ</span>
            </button>
          </div>"""

eb_code = eb_code.replace(export_old, export_new, 1)

# Add cards container right above table container
table_cont_old = """      <!-- Main entry table -->
      <div class="glass-card rounded-3xl border border-white/5 shadow-xl overflow-hidden">"""

table_cont_new = """      <!-- Mobile Cards View for Egg & Banana -->
      <div id="eggbanana-cards-container" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 hidden no-print"></div>

      <!-- Main entry table -->
      <div id="eggbanana-table-container" class="glass-card rounded-3xl border border-white/5 shadow-xl overflow-hidden">"""

eb_code = eb_code.replace(table_cont_old, table_cont_new, 1)

# Add JS state variables
js_state_old = "    let entrySortCol = 'name_english';"
js_state_new = """    let entrySortCol = 'name_english';
    let currentEggBananaViewMode = localStorage.getItem('eggbanana_view_mode') || 'cards';
    let currentEggBananaStudents = [];
    let currentEggBananaRecordsMap = {};"""

eb_code = eb_code.replace(js_state_old, js_state_new, 1)
...
render_replacement = """function setEggBananaViewMode(mode, triggerRender = true) {
      currentEggBananaViewMode = mode || 'cards';
      localStorage.setItem('eggbanana_view_mode', currentEggBananaViewMode);

      const btnCards = document.getElementById('btn-eb-cards');
      const btnTable = document.getElementById('btn-eb-table');
    ...
    function renderActiveEggBananaView() {
        ...
        const cardHtml = `
          <div class="glass-card rounded-2xl p-4 border border-white/10 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-3.5" data-card-eb-id="${s.id}">
            ...
            <div class="flex items-center justify-between pt-2 border-t border-white/10 text-xs">
              <span class="text-[10px] text-slate-400">ಆಯ್ಕೆ: <strong class="text-white">${pref}</strong></span>
        ...
        html += `
          <tr class="hover:bg-slate-50/5 border-b border-white/5 text-slate-300 font-medium">
            ...
            <td class="p-2.5 bg-indigo-500/[0.01]">
              <select id="pref-${s.id}" onchange="autoSaveRecord('${s.id}')" class="bg-black/60 border border-white/10 rounded-xl px-2.5 py-1 text-xs font-bold text-indigo-300 focus:outline-none cursor-pointer">
                ${prefOptions}
              </select>
            </td>

            <td class="p-2.5 bg-emerald-500/[0.01] border-x border-white/5 text-center">
              <input type="checkbox" id="status-${s.id}" onchange="autoSaveRecord('${s.id}')" ${isDist ? 'checked' : ''} class="w-5 h-5 rounded text-emerald-600 bg-black border-white/10 cursor-pointer transform scale-110">
            </td>
...
eb_code = re.sub(render_target, render_replacement, eb_code)

with open('EggBananaDistribution.html', 'w', encoding='utf-8') as f:
    f.write(eb_code)

print("Successfully integrated Mobile Card View into EggBananaDistribution.html")
